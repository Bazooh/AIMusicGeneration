...
class midiread(MidiOutStream):
  ...
  def create_piano_roll(self, r=(21, 109), dt=0.2):
    length = int(numpy.ceil(max(list(zip(*self.notes))[2]) / dt)) if self.notes else 0
    piano_roll = numpy.zeros((length, r[1] - r[0]), dtype=bool)
    for n in self.notes:
      if r[0] <= n[0] < r[1]:
        piano_roll[int(numpy.ceil(n[1] / dt)) : int(numpy.ceil(n[2] / dt)), n[0] - r[0]] = True
    return piano_roll
